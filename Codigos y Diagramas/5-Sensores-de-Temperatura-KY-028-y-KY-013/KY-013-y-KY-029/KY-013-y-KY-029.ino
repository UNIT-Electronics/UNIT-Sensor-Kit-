...
void loop() {
  ...
  valor = map(valor, 0, 270, 0, 50); // Mapeo del rango de entrada (0-270) al rango de temperatura (0 a 50 °C)
  ...
  Serial.println(TC);  // Impresión del valor de la temperatura en Celsius
  ...
  if (TC >= 22) {  // Si la temperatura es mayor o igual a 22°C
    digitalWrite(ledR, HIGH); // Encendido del LED Rojo
  } else {  // Si la temperatura es menor a 22°C
